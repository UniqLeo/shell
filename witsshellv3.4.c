...
void addPath(char **pathArray, char *text[], int pathSize, int startPoint){
	...
	for(int i=startPoint;i<pathSize;i++){
		...
		pathArray[i] = malloc(1024*sizeof(char));
...
void execPar(char* text[],int words, char **pathArray, int pathSize){
	if(pathArray==NULL || *pathArray==NULL  ){
		throwError();
	}
	else{
		//printf("in exec par else, see text\n");
		//printComm(text, words);
		int pids[MAX_SIZE];
		int cmdIdx =0;
		int i =0;

		while(i<words){//for each comm
			//printf("In exec par in while comm: %s\n",text[i]);
			if(strcmp(text[i],"&")==0){
				i++;
				continue;
			}

			//find end of specific command args
			int start = i;
			while(i < words && strcmp(text[i],"&")!=0){
				i++;
			}
            ...
            char fullPath[MAX_SIZE];  // Buffer for full command path

            // Build the full command path: pathArray[s] + args[0]
            snprintf(fullPath, sizeof(fullPath), "%s/%s", pathArray[s], args[0]);

				if(access(fullPath, X_OK)==0){
					//printf("in if access, before fork\n");
					int pid = fork();
					if(pid ==0){//child
				...
				if(strcmp(command, "exit")==0 || strcmp(command, "cd")==0 || strcmp(command, "path")==0){
					...
					if((pathSize<=1 ) && strstr(command,sh)!=NULL){//path hasnt been set and arg is shell script
						...
						if(findChar(text,words,RedirTarget,&RedirTargetIndex)){
							...
							//printComm(text,words);
							// if(words!=1){
							//printf("in main, in par\n");
							execPar(text,words,pathArray,pathSize);	
							// }
							
						}
						else{
							//printf("in main, plain exec\n");
				...
				while((read= getline(&line, &len, file))!= -1) {//for each line
					...
					if(strcmp(command, "exit")==0 || strcmp(command, "cd")==0 || strcmp(command, "path")==0){
						...
						if((pathSize<=1 ) && strstr(command,sh)!=NULL){//path hasnt been set and arg is shell script
							...
							else if(findChar(inputLine,len_words,ParrTarget,&ParrTargetIndex)){
								//parallel command
								execPar(inputLine,len_words,pathArray,pathSize);
